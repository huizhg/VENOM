...
class AdvImageSet(Dataset):
    ...
    def __init__(self, root):
        ...
        #self.image_paths = natsorted(self.image_paths, alg=ns.PATH)[:64]
...
class CleanImageSet(Dataset):
    ...
    def __init__(self, img_dir, label_dir):
        ...
        self.images = [self.read_image(path) for path in self.image_paths]
        #self.labels = self.read_labels(self.label_path)[:64]
        self.labels = self.read_labels(self.label_path)
...
def get_clean_images_loders(img_dir, label_dir, batch_size):
    clean_data = CleanImageSet(img_dir, label_dir)
    data_loader = DataLoader(clean_data, batch_size, shuffle=False, num_workers=8)
    return data_loader
