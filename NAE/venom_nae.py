...
def get_target_label_stamo(logits, label): # seond-like label for attack
...
def ddim_sample_no_adv(latents, context, model, timesteps: int, guidance_scale:float=2.5, eta:float=0.0):
...
def ddim_sample_adv_momentum_no_text_target(latents, context, model, vic_model, timesteps: int, guidance_scale:float=2.5, eta:float=0.0, label=None, iterations: int=5, s:float=1.0, a:float=0.5, beta=0.5):
    ...
    for k in range(iterations):   
        ...
        success_num = (pred == label).sum().item()
        print(pred)
        print(f"Success {1-success_num} / {label.shape[0]}")
        if success_num < 1 : # early exit
            break
        pri_latents = pri_latents + a * gradient.float()
        gc.collect()
